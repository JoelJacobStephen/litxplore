...
export default function PaperAnalyzerPage() {
  ...
  const [keyInsightsLoaded, setKeyInsightsLoaded] = useState(false);
  const [keyInsightsError, setKeyInsightsError] = useState<string | null>(null);
  ...
  // Load Key Insights when tab is opened
  useEffect(() => {
    if (
      ...
      loadKeyInsights().catch((err) => {
        const errorMsg =
          err instanceof Error ? err.message : "Failed to load key insights";
        setKeyInsightsError(errorMsg);
        toast.error("Key Insights Failed", errorMsg);
      });
    }
  }, [
    activeTab,
  ...
  useEffect(() => {
    if (
      ...
      loadInDepth().catch((err) => {
        const errorMsg =
          err instanceof Error ? err.message : "Failed to load in-depth analysis";
  ...
  const handleRetryKeyInsights = () => {
    setKeyInsightsError(null);
    setKeyInsightsLoaded(false);
  };
      ...
      >
        ...
        >
          ...
          <TabsTrigger
            ...
            className="rounded-none border-b-2 border-transparent data-[state=active]:border-yellow-500 data-[state=active]:text-yellow-600 dark:data-[state=active]:text-secondary-foreground text-base px-6 h-full hover:text-secondary-foreground"
            role="tab"
            aria-selected={activeTab === "key-insights"}
          >
            Key Insights
            {isLoadingKeyInsights && (
              <Loader2 className="h-4 w-4 animate-spin ml-2" />
          ...
          <div className="h-full overflow-y-auto p-4">
            {inDepthError ? (
              ...
              <div className="flex items-center justify-center h-64">
                <div className="flex flex-col items-center gap-4">
                  <Loader2 className="h-8 w-8 animate-spin text-primary" />
                  <p className="text-sm text-muted-foreground">Loading comprehensive analysis...</p>
                </div>
              </div>
            ) : analysis.in_depth ? (
        ...
        >
          <div className="h-full overflow-y-auto p-4">
            {keyInsightsError ? (
              <ErrorFallback
                title="Failed to Load Key Insights"
                message={keyInsightsError}
                onRetry={handleRetryKeyInsights}
                showRetry={true}
              />
            ) : isLoadingKeyInsights ? (
              <KeyInsightsSkeleton />
            ) : analysis.key_insights ? (
              <KeyInsightsPanel insights={analysis.key_insights} />
            ) : (
              <div className="text-center py-8">
                <p className="text-sm text-muted-foreground">
        ...
        >
          ...
          <div className="h-full">
            <ChatPanel
              paperId={paperId}
              suggestedQuestions={getFallbackQuestionsIfMissing(analysis.suggested_questions)}
            />
          </div>
        </TabsContent>
      </Tabs>
